...
class CraftMetaFirework extends CraftMetaItem implements FireworkMeta {
    ...
    CraftMetaFirework(CompoundTag tag) {
        ...
        this.power = (int) fireworks.getByte(CraftMetaFirework.FLIGHT.NBT);

        if (!fireworks.contains(CraftMetaFirework.EXPLOSIONS.NBT)) {
            return;
        }

        ListTag fireworkEffects = fireworks.getList(CraftMetaFirework.EXPLOSIONS.NBT, CraftMagicNumbers.NBT.TAG_COMPOUND);
    ...
    static FireworkEffect getEffect(CompoundTag explosion) {
        FireworkEffect.Builder effect = FireworkEffect.builder()
                .flicker(explosion.getBoolean(CraftMetaFirework.EXPLOSION_FLICKER.NBT))
                .trail(explosion.getBoolean(CraftMetaFirework.EXPLOSION_TRAIL.NBT))
                .with(CraftMetaFirework.getEffectType(0xff & explosion.getByte(CraftMetaFirework.EXPLOSION_TYPE.NBT)));
        ...
        for (int color : explosion.getIntArray(CraftMetaFirework.EXPLOSION_FADE.NBT)) {
    ...
    static CompoundTag getExplosion(FireworkEffect effect) {
        ...
        explosion.putByte(CraftMetaFirework.EXPLOSION_TYPE.NBT, (byte) CraftMetaFirework.getNBT(effect.getType()));
