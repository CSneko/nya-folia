        ...
        while ( !this.stopping )
        {
            ...
            if ( this.lastTick != 0 && this.timeoutTime > 0 && WatchdogThread.hasStarted && (!server.isRunning() || (currentTime > this.lastTick + this.earlyWarningEvery && !DISABLE_WATCHDOG) )) // Paper - add property to disable
            {
                ...
                // Paper end
                    ...
                    log.log( Level.SEVERE, "主人，服务器无响应了喵，这 (可能) 不是 Paper 的问题。" );
                    log.log( Level.SEVERE, "如果你在下面的服务器线程转储中看到插件，请向该作者报告此问题哦，特别是如果看起来像是 HTTP 或 MySQL 操作。" );
                    log.log( Level.SEVERE, "如果你看到了世界保存或编辑，那就说明你做的事情太多啦，服务器带不动了喵" );
                    log.log( Level.SEVERE, "实在不行可以增加 spigot.yml 中的超时时间哦，但请注意哦，这将用大型延迟代替崩溃喵" );
                    log.log( Level.SEVERE, "如果你觉得这是nya-Folia的问题，请将其报告给 https://github.com/CSneko/nya-folia" );
                    log.log( Level.SEVERE, "记得一定要包含所有相关的控制台错误和 Minecraft 崩溃报告哦，这很重要的喵" );
                    log.log( Level.SEVERE, "nya-Folia 版本：" + Bukkit.getServer().getVersion() + " 喵~");
                //
                ...
                {
                    log.log(Level.SEVERE, "--- 不要报告这个问题哦 - THIS IS NOT A BUG OR A CRASH  - " + Bukkit.getServer().getVersion() + " ---");
                ...
                for ( ThreadInfo thread : threads )
                {
                    ...
                    log.log(Level.SEVERE, "--- 不要报告这个问题哦，这不是错误或崩溃哦 ---");
                }
