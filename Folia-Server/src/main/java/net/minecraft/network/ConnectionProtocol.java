...
public enum ConnectionProtocol {
    ...
    private ConnectionProtocol(String stateId, ConnectionProtocol.ProtocolBuilder initializer) {
        this.id = stateId;
        this.flows = initializer.buildCodecs(this);
    }

    @VisibleForDebug
    public Int2ObjectMap<Class<? extends Packet<?>>> getPacketsByIds(PacketFlow side) {
        return this.flows.get(side).packetsByIds();
    }
    ...
    public ConnectionProtocol.CodecData<?> codec(PacketFlow side) {
        return this.flows.get(side);
    }
    ...
    public static class CodecData<T extends PacketListener> implements BundlerInfo.Provider {
        ...
        public CodecData(ConnectionProtocol state, PacketFlow side, ConnectionProtocol.PacketSet<T> backingHandler) {
            this.protocol = state;
            this.flow = side;
            this.packetSet = backingHandler;
        }
        ...
        Int2ObjectMap<Class<? extends Packet<?>>> packetsByIds() {
            ...
            this.packetSet.classToId.forEach((packet, id) -> {
                int2ObjectMap.put(id.intValue(), packet);
            });
        ...
        public Packet<?> createPacket(int id, FriendlyByteBuf buf) {
            return this.packetSet.createPacket(id, buf);
        }
    ...
    static class PacketSet<T extends PacketListener> {
        ...
        final Object2IntMap<Class<? extends Packet<? super T>>> classToId = Util.make(new Object2IntOpenHashMap<>(), (map) -> {
        ...
        public <P extends Packet<? super T>> ConnectionProtocol.PacketSet<T> addPacket(Class<P> type, Function<FriendlyByteBuf, P> packetFactory) {
        ...
        public boolean isKnownPacket(Class<?> clazz) {
            return this.classToId.containsKey(clazz) || this.extraClasses.contains(clazz);
        }
    ...
    static class ProtocolBuilder {
        ...
        public Map<PacketFlow, ConnectionProtocol.CodecData<?>> buildCodecs(ConnectionProtocol state) {
            ...
            for(PacketFlow packetFlow : PacketFlow.values()) {
                ...
                map.put(packetFlow, new ConnectionProtocol.CodecData<>(state, packetFlow, packetSet));
            }
